...
export const getAllOrders = async (req, res, next) => {
  try {
    const { page = 1, limit = 20, status, orderStatus, store_id, search, startDate, endDate } = req.query;
    ...
    // Filter by status (support both 'status' and 'orderStatus')
    ...
    if (store_id) {
      conditions += ' AND o.store_id = ?';
      params.push(store_id);
    }

    if (search) {
      conditions += ' AND (o.id LIKE ? OR u.email LIKE ? OR u.full_name LIKE ?)';
    ...
    // Count query
    const countSql = `SELECT COUNT(*) as total ${builtFrom} ${conditions}`;
    const [{ total }] = await query(countSql, params);
    ...
    const orders = await query(dataSql, dataParams);
    const mappedOrders = orders.map(mapOrderResponse);
